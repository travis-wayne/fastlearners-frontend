...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      const r = await fetch(`${UPSTREAM_BASE}/student/classes`, {
        method: "GET",
        headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          const retryR = await fetch(`${UPSTREAM_BASE}/student/classes`, {
            method: "GET",
            headers: {
              Accept: "application/json",
              Authorization: `Bearer ${auth.token}`,
          ...
          return handleApiError(retryError, "Network error: Failed to fetch classes after retry", requestId);
        }
      }
    ...
    return handleApiError(e, "Failed to fetch classes", requestId);
  }
}
