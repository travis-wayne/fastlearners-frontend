...
async function checkUsernameAvailability(
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_DEBUG_AUTH === "true") {
      console.warn(
        "[profile/edit] Username availability check skipped:",
        error,
      );
    }
    return { available: true };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_DEBUG_AUTH === "true") {
        console.warn(
          "Failed to fetch current profile for validation:",
          profileError,
        );
      }
    }
    ...
    const hasOnlyAcademicFields =
      body.class !== undefined &&
      Object.keys(body).filter(
        (k) => !["class", "discipline", "term"].includes(k),
      ).length === 0;
    ...
    if (!body.role && currentProfile) {
      const currentRole = Array.isArray(currentProfile.role)
    ...
    const validationResult = validateProfileEdit(
      currentProfile || {},
      updateData,
    );
    ...
    if (!validationResult.isValid) {
      ...
      if (
        validationResult.errors.role?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 401;
      } else if (
        validationResult.errors.class?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 400;
      } else if (
        validationResult.errors.discipline?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 401;
        ...
        validationResult.errors.discipline?.some((msg) =>
          msg.includes("SSS class"),
        )
      ) {
        statusCode = 400;
      } else if (
        validationResult.errors.username?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 401;
      } else if (
        validationResult.errors.date_of_birth?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 401;
      } else if (
        validationResult.errors.gender?.some((msg) =>
          msg.includes("already updated"),
        )
      ) {
        statusCode = 401;
      ...
      return NextResponse.json(
        {
          ...
          message:
            statusCode === 422
              ? "Validation failed."
              : Object.values(validationResult.errors)[0]?.[0] ||
                "Validation failed.",
            ...
            return NextResponse.json(
              {
                success: false,
                message:
                  "Role already updated and cannot be changed. For further enquiries, please contact our support team.",
                errors: null,
                requestId,
              },
              { status: 401 },
            ...
            return NextResponse.json(
              {
                success: false,
                message:
                  "Class already updated. Make a request for class upgrade.",
                errors: null,
                requestId,
              },
              { status: 400 },
        ...
        try {
          ...
          if (!retryR.ok) {
            ...
            if (retryData?.message) {
              ...
              if (
                retryData.message.includes("SSS class to choose a discipline")
              ) {
                return NextResponse.json(
                  {
                    success: false,
                    message:
                      "You have to be in SSS class to choose a discipline!",
                    errors: null,
                    requestId,
                  },
                  { status: 400 },
          ...
          return handleApiError(
            retryError,
            "Network error: Failed to update profile after retry",
            requestId,
          );
        }
      }
