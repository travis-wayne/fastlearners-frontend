...
async function testUserAndStudentEndpoints() {
  ...
  try {
    ...
    if (profileRes.data.success) {
      ...
      console.log(
        "User:",
        profileRes.data.content.user.name,
        `(ID: ${profileRes.data.content.user.id})`,
      );
    } else {
  ...
  try {
    ...
    if (profileDataRes.data.success) {
      ...
      console.log(
        "Classes available:",
        profileDataRes.data.content.classes.length,
      );
    } else {
      console.error(
        "❌ Failed to fetch profile data:",
        profileDataRes.data.message,
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (dashboardRes.data.success) {
      ...
      console.log(
        "Content:",
        JSON.stringify(dashboardRes.data.content, null, 2),
      );
    } else {
      console.error(
