...
export async function GET(
  req: NextRequest,
  { params }: { params: { subjectId: string; termId: string } }
) {
  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/scores/lessons/total/${subjectId}/${termId}`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch all lessons total scores after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to fetch all lessons total scores", requestId);
  }
}
