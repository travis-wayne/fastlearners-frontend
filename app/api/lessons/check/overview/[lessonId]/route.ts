  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/check/overview/${lessonId}`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to check overview completion after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to check overview completion", requestId);
  }
}
