...
import { Concept, LessonContent } from "@/lib/types/lessons";
  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      ...
      if (canProceed) {
        ...
        const { sectionProgress: currentSectionProgress } =
          useLessonsStore.getState();

        // Re-validate completion with fresh state
        const isActuallyComplete = validSectionIds.every(
          (id) => currentSectionProgress[id]?.isCompleted,
        );

        if (!moved && isLastStep && selectedLesson?.id && !celebrationShown) {
          // Self-healing: Check if we can verify missing sections on the fly
          const missing = validSectionIds.filter(
            (id) => !currentSectionProgress[id]?.isCompleted,
          );
          let healedCount = 0;
            ...
            for (const sectionId of missing) {
              try {
                ...
                if (sectionId === "summary_application") {
                  const { checkLessonSummaryAndApplication } =
                    await import("@/lib/api/lessons");
                  const res = await checkLessonSummaryAndApplication(
                    selectedLesson.id,
                  );
                  if (res?.success && res?.content?.check?.is_completed)
                    verified = true;
                } else if (sectionId === "overview") {
                  const { checkLessonOverview } =
                    await import("@/lib/api/lessons");
          ...
          const { sectionProgress: updatedProgress } =
            useLessonsStore.getState();
          const isNowComplete = validSectionIds.every(
            (id) => updatedProgress[id]?.isCompleted,
          );
          ...
          if (isNowComplete) {
            ...
            const remainingMissing = validSectionIds.filter(
              (id) => !updatedProgress[id]?.isCompleted,
            );
            console.warn(
              "Cannot finish lesson. Missing sections:",
              remainingMissing,
            );
