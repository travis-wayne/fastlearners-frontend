  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/student/guardian/request/reject/${id}`,
        {
          method: "GET",
          headers: {
      ...
      if (
        ...
        try {
          ...
          return NextResponse.json(retryData || { success: true }, {
            status: retryUpstream.status,
          });
        } catch (retryError) {
          return handleApiError(
            retryError,
            "Network error: Failed to reject guardian request after retry",
            requestId,
          );
        }
      }
    ...
    return handleApiError(err, "Failed to reject guardian request", requestId);
  }
}
