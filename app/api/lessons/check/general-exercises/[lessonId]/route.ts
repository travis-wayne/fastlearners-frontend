...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to check general exercises completion after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to check general exercises completion", requestId);
  }
}
