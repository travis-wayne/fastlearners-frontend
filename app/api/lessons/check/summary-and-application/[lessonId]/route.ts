...
export async function GET(
  ...
  try {
    ...
    try {
      // Note: Upstream endpoint uses snake_case 'summary_application'
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/check/summary_application/${lessonId}`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to check summary and application completion after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to check summary and application completion", requestId);
  }
}
