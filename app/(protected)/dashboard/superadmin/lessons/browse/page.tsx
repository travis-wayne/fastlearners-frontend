...
import { ArrowLeft, BookOpen, Filter, Search, Target, TrendingUp } from "lucide-react";
...
export default function BrowseLessonsPage() {
  ...
  const filteredLessons = useMemo(() => {
    ...
    return lessons.filter((l) =>
      [l.topic, l.class, l.subject, l.term].some((v) => v?.toLowerCase?.().includes(q)),
    );
  }, [lessons, searchQuery]);
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="container mx-auto space-y-6 p-4 md:p-6">
      <motion.div variants={itemVariants} className="flex items-center gap-4">
        ...
        <DashboardHeader heading="Browse Lessons" text="Search and view all lesson content" />
      </motion.div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2"><BookOpen className="size-4 text-blue-600" /><span className="text-sm text-muted-foreground">Results</span></div>
              <p className="mt-1 text-2xl font-bold">{filteredLessons.length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2"><Target className="size-4 text-green-600" /><span className="text-sm text-muted-foreground">Selected Subject</span></div>
              <p className="mt-1 text-2xl font-bold">{metadata?.subjects?.find(s=>s.id.toString()===selectedSubject)?.name || "-"}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2"><TrendingUp className="size-4 text-purple-600" /><span className="text-sm text-muted-foreground">Term • Week</span></div>
              <p className="mt-1 text-2xl font-bold">{metadata?.terms?.find(t=>t.id.toString()===selectedTerm)?.name || "-"} • {metadata?.weeks?.find(w=>w.id.toString()===selectedWeek)?.name || "-"}</p>
            </CardContent>
          </Card>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants}>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Filter className="size-5" />Filter Lessons</CardTitle>
            <CardDescription>Select class, subject, term and week to find lessons</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-col gap-4 md:flex-row">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                <Input placeholder="Search lessons..." value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} className="pl-10" />
              </div>
                ...
                <Select value={selectedWeek} onValueChange={setSelectedWeek}>
                  ...
                  <SelectContent>
                    {metadata?.weeks?.map((w)=> (
                      <SelectItem key={w.id} value={w.id.toString()}>Week {w.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
      ...
      <motion.div variants={itemVariants}>
        <Tabs defaultValue="all" className="space-y-4">
          <TabsList className="grid w-full grid-cols-1 md:grid-cols-3">
            <TabsTrigger value="all">All Results ({filteredLessons.length})</TabsTrigger>
          ...
          <TabsContent value="all">
            {filteredLessons.length === 0 ? (
              <Card><CardContent className="py-12 text-center"><BookOpen className="mx-auto mb-4 size-12 text-muted-foreground" /><h3 className="mb-2 text-lg font-semibold">No lessons found</h3><p className="text-muted-foreground">Try different filter combinations to find lessons.</p></CardContent></Card>
            ) : (
...
<Card key={lesson.id} className="transition-shadow hover:shadow-md">
                    <CardHeader>
                      ...
                      <CardDescription>{lesson.class} • {lesson.subject} • {lesson.term} • Week {lesson.week}</CardDescription>
                    </CardHeader>
                    <CardContent className="flex justify-between gap-2">
                      <Link href={`/dashboard/superadmin/lessons/${lesson.id}`}>
                        <Button size="sm">Open</Button>
                      </Link>
                      <Link href={`/dashboard/superadmin/lessons/${lesson.id}/content`}>
          ...
          <TabsContent value="by-subject">
            {Object.entries(
              filteredLessons.reduce((acc: Record<string, typeof filteredLessons>, l) => {
...
<Card key={lesson.id} className="transition-shadow hover:shadow-md">
                      <CardHeader>
                        <CardTitle className="text-lg">{lesson.topic}</CardTitle>
                        <CardDescription>{lesson.class} • {lesson.term} • Week {lesson.week}</CardDescription>
                      </CardHeader>
                      <CardContent className="flex justify-between gap-2">
                        <Link href={`/dashboard/superadmin/lessons/${lesson.id}`}><Button size="sm">Open</Button></Link>
                        <Link href={`/dashboard/superadmin/lessons/${lesson.id}/content`}><Button size="sm" variant="outline">Content</Button></Link>
                      </CardContent>
                    </Card>
                  ))}
                </div>
          ...
          <TabsContent value="by-week">
            {Object.entries(
              filteredLessons.reduce((acc: Record<string, typeof filteredLessons>, l) => {
                const key = `Week ${l.week}`; (acc[key] ||= []).push(l); return acc;
              }, {}),
            ).map(([week, items]) => (
<div key={week} className="mb-6 space-y-3">
                <h3 className="text-sm font-semibold text-muted-foreground">{week}</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {items.map((lesson) => (
<Card key={lesson.id} className="transition-shadow hover:shadow-md">
                      <CardHeader>
                        <CardTitle className="text-lg">{lesson.topic}</CardTitle>
                        <CardDescription>{lesson.class} • {lesson.subject} • {lesson.term}</CardDescription>
                      </CardHeader>
                      <CardContent className="flex justify-between gap-2">
                        <Link href={`/dashboard/superadmin/lessons/${lesson.id}`}><Button size="sm">Open</Button></Link>
                        <Link href={`/dashboard/superadmin/lessons/${lesson.id}/content`}><Button size="sm" variant="outline">Content</Button></Link>
