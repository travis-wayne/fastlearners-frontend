...
import { BarChart3, TrendingUp, Award, FileText, Calendar, Download } from "lucide-react";
...
const mockGrades = [
  { subject: "Mathematics", ca: 85, exam: 78, total: 81.5, grade: "A", position: 3, remarks: "Excellent" },
  { subject: "English", ca: 78, exam: 82, total: 80, grade: "A", position: 5, remarks: "Very Good" },
  { subject: "Basic Science", ca: 88, exam: 76, total: 82, grade: "A", position: 2, remarks: "Excellent" },
  { subject: "Social Studies", ca: 75, exam: 68, total: 71.5, grade: "B", position: 8, remarks: "Good" },
  { subject: "Civic Education", ca: 82, exam: 85, total: 83.5, grade: "A", position: 1, remarks: "Outstanding" },
];
...
export default function RecordsPage() {
      ...
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">Please select your class and term to view records.</p>
  ...
  const overallAverage = mockGrades.reduce((sum, grade) => sum + grade.total, 0) / mockGrades.length;
  const currentTermRecord = mockTerms.find(t => t.term === "1st Term") || mockTerms[0];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track your academic performance for {classDisplay}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-sm text-muted-foreground">out of {currentTermRecord.total}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">{mockGrades.filter(g => g.grade === 'A').length}</p>
                  ...
                  {mockGrades.map((grade, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-center">
                        <Badge variant={grade.grade === 'A' ? 'default' : grade.grade === 'B' ? 'secondary' : 'outline'}>
                          {grade.grade}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center">{grade.position}</TableCell>
                      <TableCell>
                        <span className={`text-sm ${
                          grade.remarks === 'Outstanding' || grade.remarks === 'Excellent' ? 'text-green-600' :
                          grade.remarks === 'Very Good' ? 'text-blue-600' :
                          grade.remarks === 'Good' ? 'text-yellow-600' : 'text-gray-600'
                        }`}>
                          {grade.remarks}
                        </span>
                      </TableCell>
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your academic performance across different terms</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {mockTerms.map((term, index) => (
                  <Card key={index} className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge variant="outline">{term.average.toFixed(1)}% Average</Badge>
                    </div>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-muted-foreground">Class Position: </span>
                        <span className="font-medium">{term.position} of {term.total}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Grade: </span>
                        <span className="font-medium">
                          {term.average >= 80 ? 'A' : term.average >= 70 ? 'B' : term.average >= 60 ? 'C' : 'D'}
                        </span>
                      </div>
                    </div>
