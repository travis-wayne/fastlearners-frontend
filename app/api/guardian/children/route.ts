...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      const upstream = await fetch(`${UPSTREAM_BASE}/guardian/children`, {
        method: "GET",
        headers: {
      ...
      if (
        ...
        try {
          const retryUpstream = await fetch(`${UPSTREAM_BASE}/guardian/children`, {
            method: "GET",
            headers: {
              Accept: "application/json",
              Authorization: `Bearer ${auth.token}`,
          ...
          return handleApiError(retryError, "Network error: Failed to fetch guardian children after retry", requestId);
        }
      }
      throw fetchError;
    }
  } catch (err: any) {
    return handleApiError(err, "Failed to fetch guardian children", requestId);
  }
}
