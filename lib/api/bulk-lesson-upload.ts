 ...
 */
export const uploadAllLessonFilesBulk = async (files: BulkUploadFiles): Promise<BulkUploadResult> => {
  try {
    ...
    const missingFiles = requiredFields.filter(field => !files[field as keyof BulkUploadFiles]);
    ...
    console.log('🎯 Uploading to:', `${BASE_URL}/superadmin/lessons/uploads/all-lesson-files`);
    ...
    return {
      success: true,
      message: response.data.message || 'All lesson files uploaded successfully',
    ...
    if (error.response?.data) {
      ...
      // Handle specific error codes from documentation
        ...
        errorMessage = responseData.message || 'Invalid CSV format or missing columns';
      }
      else if (error.response.status === 404) {
        errorMessage = responseData.message || 'Resource not found (class, subject, term, week, lesson, or concept)';
      }
      else if (error.response.status === 500) {
        errorMessage = responseData.message || 'Server error processing CSV files';
      }
 ...
 */
export const validateBulkUploadFiles = (files: Partial<BulkUploadFiles>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
