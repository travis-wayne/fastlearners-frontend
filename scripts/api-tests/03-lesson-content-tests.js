...
async function testLessonContent() {
  ...
  try {
    ...
    console.log("Could not fetch profile for debug");
  }
  ...
  try {
    ...
    if (subjectsRes.data.success) {
      ...
      console.log(
        "Available Subjects:",
        subjects.map((s) => `${s.name} (${s.slug})`).join(", "),
      );
      if (subjects.length > 0) {
        subjectSlug = subjects[0].slug;
        console.log(
          `Selected Subject: ${subjects[0].name}, Slug: ${subjectSlug}`,
        );
      } else {
  ...
  try {
    ...
    for (const sub of subjects) {
      ...
      try {
        ...
        console.log(
          `❌ Failed for ${sub.slug}: ${e.response ? e.response.data.message : e.message}`,
        );
      }
    }
  ...
  try {
    ...
    const overviewRes = await client.get(
      `/api/v1/lessons/${subjectSlug}/${topicSlug}/overview`,
    );
    if (overviewRes.data.success) {
      console.log("✅ Overview fetched successfully.");
      console.log(
        "Overview:",
        JSON.stringify(overviewRes.data.content.overview, null, 2),
      );
    } else {
  ...
  try {
    ...
    if (contentRes.data.success) {
      ...
      console.log(
        `Concepts count: ${lesson.concepts ? lesson.concepts.length : 0}`,
      );
