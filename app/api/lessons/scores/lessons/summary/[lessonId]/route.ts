  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/scores/lessons/summary/${lessonId}`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch lesson summary after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to fetch lesson summary", requestId);
  }
}
