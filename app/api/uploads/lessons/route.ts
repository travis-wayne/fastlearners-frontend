...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const lessonsFile = formData.get("lessons_file") as File;

    if (!lessonsFile) {
      return NextResponse.json(
        {
          success: false,
          message: "Missing lessons_file",
          content: null,
          code: 400,
        },
        { status: 400 }
    ...
    const upstream = await fetch(
      `${UPSTREAM_BASE}/superadmin/lessons/uploads/lessons`,
      {
        method: "POST",
        headers: {
    ...
    if (process.env.NEXT_PUBLIC_DEBUG_AUTH === "true") {
      console.error("Lessons upload API error:", err);
    }
    return NextResponse.json(
      {
        success: false,
