...
async function testAnsweringAndCompletion() {
  ...
  console.log(
    `--- Testing Answering & Completion for ${subjectSlug}/${topicSlug} ---`,
  );
  ...
  try {
    ...
    if (contentRes.data.success) {
      ...
      if (lesson.concepts && lesson.concepts.length > 0) {
        ...
        if (
          lesson.concepts[0].exercises &&
          lesson.concepts[0].exercises.length > 0
        ) {
  ...
  if (exerciseId) {
    try {
      ...
      if (
        ...
        console.log(
          `ℹ️ Answer check returned ${err.response.status}: ${err.response.data.message}`,
        );
      } else {
  ...
  if (generalExerciseId) {
    try {
      ...
      const genCheckRes = await client.post(
      ...
      if (genCheckRes.data.success) {
        console.log(
          "✅ General Answer checked successfully:",
          genCheckRes.data.message,
        );
      } else {
        console.log(
          "❌ General Answer check failed (logic):",
          genCheckRes.data.message,
        );
      }
    } catch (err) {
      if (
        ...
        console.log(
          `ℹ️ General Answer check returned ${err.response.status}: ${err.response.data.message}`,
        );
      } else {
  ...
  if (lessonId) {
    try {
      ...
      const completeRes = await client.get(
        `/api/v1/lessons/check/overview/${lessonId}`,
      );
      if (completeRes.data.success) {
        ...
        console.error(
          "❌ Overview completion check failed:",
