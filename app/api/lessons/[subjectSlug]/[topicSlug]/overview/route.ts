  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/${subjectSlug}/${topicSlug}/overview`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch topic overview after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to fetch topic overview", requestId);
  }
}