...
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ lessonId: string; childId: string }> }
) {
  const { lessonId, childId } = await params;
  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/guardian/children/lesson/${lessonId}/${childId}/view-lesson-details`,
        {
          method: "GET",
          headers: {
      ...
      if (
        ...
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch lesson details after retry", requestId);
        }
      }
      throw fetchError;
    }
  } catch (err: any) {
    return handleApiError(err, "Failed to fetch lesson details", requestId);
  }
}
