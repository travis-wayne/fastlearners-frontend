...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validation
    if (
      !body.general_exercise_id ||
      typeof body.general_exercise_id !== "number" ||
      body.general_exercise_id <= 0
    ) {
      return createErrorResponse(
        "Invalid general exercise ID",
        422,
        undefined,
        requestId,
      );
    ...
    try {
      ...
      if (!upstream.ok) {
        if (process.env.NODE_ENV === "development") {
          console.log(
            "[check-general-exercise-answer] Returning error to client:",
            {
              status: upstream.status,
              data,
            },
      ...
      if (
        ...
        try {
          ...
          return handleApiError(
            retryError,
            "Network error: Failed to check general exercise answer after retry",
            requestId,
          );
        }
      }
    ...
    return handleApiError(
      err,
      "Failed to check general exercise answer",
      requestId,
    );
  }
}
