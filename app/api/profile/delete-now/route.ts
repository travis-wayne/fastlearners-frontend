...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    try {
      const r = await fetch(`${UPSTREAM_BASE}/profile/delete-now`, {
        method: "DELETE",
        headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          const retryR = await fetch(`${UPSTREAM_BASE}/profile/delete-now`, {
            method: "DELETE",
            headers: {
              Accept: "application/json",
              Authorization: `Bearer ${auth.token}`,
          ...
          return handleApiError(retryError, "Network error: Failed to delete account now after retry", requestId);
        }
      }
    ...
    return handleApiError(e, "Delete account now request failed", requestId);
  }
}
