...
export function useRBACGuard(options: UseRBACGuardOptions = {}) {
  ...
  const canAccess = userRole ? RBACUtils.canAccessRoute(userRole, pathname) : false;
...
export function useFeatureAccess(feature: string) {
  ...
  const canAccessFeature = user ? user.role.some(role => {
...
export function usePermissionCheck() {
  ...
  const hasPermission = (permission: string): boolean => {
    if (!userRole) {
      console.log(`❌ [PERMISSION DEBUG] No userRole for permission "${permission}"`);
      return false;
    }
    
    console.log(`🔍 [PERMISSION DEBUG] Checking permission "${permission}" for role "${userRole}"`);
    ...
    switch (permission) {
      case 'manage_lessons':
        ...
        console.log(`   - RBACUtils.hasPermission(${userRole}, 'TEACHER') = ${result}`);
        break;
      case 'manage_users':
        result = RBACUtils.hasPermission(userRole, 'ADMIN');
        console.log(`   - RBACUtils.hasPermission(${userRole}, 'ADMIN') = ${result}`);
        break;
      case 'system_config':
        result = RBACUtils.hasPermission(userRole, 'SUPERADMIN');
        console.log(`   - RBACUtils.hasPermission(${userRole}, 'SUPERADMIN') = ${result}`);
        break;
      case 'assign_roles':
        ...
        break;
      case 'view_progress':
        result = ['student', 'guardian', 'teacher', 'admin', 'superadmin'].includes(userRole);
      ...
      case 'view_admin_panel':
        result = RBACUtils.hasPermission(userRole, 'ADMIN');
        console.log(`   - RBACUtils.hasPermission(${userRole}, 'ADMIN') for admin panel = ${result}`);
        break;
      case 'create_user':
        result = RBACUtils.hasPermission(userRole, 'ADMIN');
        console.log(`   - RBACUtils.hasPermission(${userRole}, 'ADMIN') for user creation = ${result}`);
        break;
      default:
        result = false;
    ...
    console.log(`🎯 [PERMISSION DEBUG] Final result for "${permission}": ${result}`);
    return result;
  };
