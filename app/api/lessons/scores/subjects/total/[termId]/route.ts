...
export async function GET(
  ...
  try {
    ...
    if (!termId) {
      return createErrorResponse("Invalid request: termId is required", 400, undefined, requestId);
    }
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/lessons/scores/subjects/total/${termId}`,
        {
          method: "GET",
          headers: {
      ...
      if (fetchError.name === 'AbortError' || fetchError.message?.includes('fetch')) {
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch all subjects total scores after retry", requestId);
        }
      }
    ...
    return handleApiError(err, "Failed to fetch all subjects total scores", requestId);
  }
}
