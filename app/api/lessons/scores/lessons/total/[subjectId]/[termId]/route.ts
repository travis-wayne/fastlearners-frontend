  ...
  try {
    ...
    if (!subjectId || !termId) {
      return createErrorResponse(
        "Invalid request: subjectId and termId are required",
        400,
        undefined,
        requestId,
      );
      ...
      if (
        ...
        try {
          ...
          return handleApiError(
            retryError,
            "Network error: Failed to fetch all lessons total scores after retry",
            requestId,
          );
        }
      }
    ...
    return handleApiError(
      err,
      "Failed to fetch all lessons total scores",
      requestId,
    );
  }
}
