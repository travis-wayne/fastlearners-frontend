    ...
    >
      ...
      {user && (
        <>
          ...
          <div>
            🎯 Primary Role:{" "}
            <span style={{ color: "#4ade80" }}>{user.role?.[0] || "None"}</span>
          </div>
          <div>
            🏷️ All Roles: {user.role ? JSON.stringify(user.role) : "None"}
          </div>

          <div style={{ marginTop: "8px" }}>
            <strong>RBAC Status:</strong>
          </div>
          <div>
            🛡️ Current Role:{" "}
            <span style={{ color: "#fbbf24" }}>{userRole || "Unknown"}</span>
          </div>
          <div>
            📋 Can Manage Lessons:{" "}
          ...
          <div>
            ⚙️ Can View Admin: {hasPermission("view_admin_panel") ? "✅" : "❌"}
          </div>
        </>
      )}
