...
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ childId: string; classId: string; subjectId: string }> }
) {
  const { childId, classId, subjectId } = await params;
  ...
  try {
    ...
    try {
      const upstream = await fetch(
        `${UPSTREAM_BASE}/guardian/children/subject/${childId}/${classId}/${subjectId}/view-subject-details`,
        {
          method: "GET",
          headers: {
      ...
      if (
        ...
        try {
          ...
          return handleApiError(retryError, "Network error: Failed to fetch subject details after retry", requestId);
        }
      }
      throw fetchError;
    }
  } catch (err: any) {
    return handleApiError(err, "Failed to fetch subject details", requestId);
  }
}
